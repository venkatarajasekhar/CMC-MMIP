...
int main(int argc, char **argv) {
	try {
		...
		else if (!strcmp(argv[3], "mirror") && argc == 5)
			ImageIO::write(argv[2], Transform::mirror(first_image, argv[4]));
		else if (!strcmp(argv[3], "rotate") && argc == 6)
			ImageIO::write(argv[2], Transform::rotate(
				first_image, argv[4], strtof(argv[5], nullptr)
			) );
		else if (!strcmp(argv[3], "up_bilinear") && argc == 5)
			ImageIO::write(argv[2], Transform::up_bilinear(
				first_image, strtof(argv[4], nullptr)
			) );
		else if (!strcmp(argv[3], "up_bicubic") && argc == 5)
			ImageIO::write(argv[2], Transform::up_bicubic(
				first_image, strtof(argv[4], nullptr)
			) );
		else if (!strcmp(argv[3], "metric") && argc == 5) {
			GrayscaleFloatImage&& second_image = ImageIO::read<float>(argv[2]);
			if (!strcmp(argv[4], "mse"))
				fprintf(stdout, "mse = %f\n", Metric::mse_metric(
					first_image, second_image
				) );
			else if (!strcmp(argv[4], "psnr"))
				fprintf(stdout, "psnr = %f\n", Metric::psnr_metric(
					first_image, second_image
				) );
			else if (!strcmp(argv[4], "ssim"))
				fprintf(stdout, "ssim = %f\n", Metric::ssim_metric(
					first_image, second_image
				) );
			else if (!strcmp(argv[4], "mssim"))
		...
		else if (!strcmp(argv[3], "sobel") && argc == 5)
			ImageIO::write(argv[2], Detection::sobel(first_image, argv[4]));
		else
			throw "Usage: ./out (first_image) (second_image) (command) [parameters...]";
		return 0;
	} catch (const char* ex) {
