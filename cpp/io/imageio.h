	...
	static Image<T> _bmp_rgb_read(BmpRead& bmp_input) {
		...
		try {
			...
			for (int i = _height - 1; i >= 0; i--) {
				...
				for (int j = 0; j < _width; j++)
					_input(i, j) = 0.114 * _row[3*j] + 0.5871 * _row[3*j + 1]
						+ 0.2989 * _row[3*j + 2];
			}
			delete[] _row;
			return _input;
	...
	static Image<T> _bmp_rgbx_read(BmpRead& bmp_input) {
		...
		try {
			...
			for (int i = _height - 1; i >= 0; i--) {
				...
				for (int j = 0; j < _width; j++)
					_input(i, j) = 0.114 * _row[4*j] + 0.5871 * _row[4*j + 1]
						+ 0.2989 * _row[4*j + 2];
			}
			delete[] _row;
			return _input;
	...
	int write(const char* filename, Image<T>&& output) {
		...
		try {
			...
			for (int i = _height - 1; i >= 0; i--) {
				for (int j = 0; j < _width; j++) {
					T value = output(i, j);
					_row[j] = (uint8_t)(value < 0 ? Image<T>::BLACK
						: (value > 255 ? Image<T>::WHITE : value));
				}
